...
fork(int n)
{
  ...
  *np->tf = *curproc->tf;
  ...
  pid = np->pid;
  if(n<5){   //valores padrao e maximo
    n = 5;
  }else if(n>100){
    n=100;
  }
...
scheduler(void)
{
  ...
  for(;;){
    ...
    acquire(&ptable.lock);
    tot_bilhetes = calculaTotal();
    if(tot_bilhetes ==0)tot_bilhetes ++;
    //gerar numero aleatorio para dentro da variavel aleat
    aleat = (rand() % tot_bilhetes)+1;
    if(aleat < 0)aleat = aleat * -1;
    if(aleat == 0)aleat = 1;
